...
class Matrix {
 protected:
  ...
  Matrix(int rows, int cols):rows_(rows),cols_(cols) {
    ...
    memset(linear_, 0, rows * cols * sizeof(T));
  }
...
class RowMatrix : public Matrix<T> {
 public:
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (size != this->GetColumnCount() * this->GetRowCount()){
      throw ExceptionType::OUT_OF_RANGE;
    }
    int i = 0;
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    std::unique_ptr<RowMatrix<T>> new_mat{new RowMatrix<T>(matrixA->GetRowCount(),matrixA->GetColumnCount())};
    for (int i = 0; i < matrixA->GetRowCount(); ++i) {
      for (int j = 0; j < matrixA->GetColumnCount(); ++j) {
        new_mat->SetElement(i,j,matrixA->GetElement(i,j) + matrixB->GetElement(i,j));
      }
    }
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    auto new_mat = new RowMatrix<T>(matrixA->GetRowCount(),matrixB->GetColumnCount());
    for (int i = 0; i < matrixA->GetRowCount(); ++i) {
      for (int j = 0; j < matrixB->GetColumnCount(); ++j) {
        for (int k = 0; k < matrixA->GetColumnCount(); ++k) {
          new_mat->SetElement(i,j,new_mat->GetElement(i,j) + matrixA->GetElement(i,k) * matrixB->GetElement(k,j));
        }
      }
    }
